...
class VectorSpace:
    ...
    @staticmethod
    def minmax_invert(coords):
        scaler = MinMaxScaler()
        transformed = scaler.fit_transform(coords)
    ...
    @staticmethod
    def robust_invert(coords):
        scaler = RobustScaler()
        transformed = scaler.fit_transform(coords)
        return -transformed

    @staticmethod
    def normalize_dimensions(data, scaling_methods):
        """
        Normalize the coordinates of points based on the specified scaling methods for each dimension.
        Parameters:
        data (np.array): An np.array specifying the coordinates of each point in the vectorspace.
        scaling_methods (dict): A dictionary specifying the scaling method for each dimension.
                                Keys are dimension indices, and values are scaling method names.
                                Methods: "minmax", "zscore", "robust", "minmax_invert", "zscore_invert", "robust_invert"
        Returns:
        data (np.array): The normalized coordinates.
        """
        for dim_index, method in scaling_methods.items():
            if method == "minmax":
                data[:, dim_index] = VectorSpace.minmax(
                    data[:, dim_index].reshape(-1, 1)
                ).flatten()
            elif method == "zscore":
                data[:, dim_index] = VectorSpace.zscore(
                    data[:, dim_index].reshape(-1, 1)
                ).flatten()
            elif method == "robust":
                data[:, dim_index] = VectorSpace.robust(
                    data[:, dim_index].reshape(-1, 1)
                ).flatten()
            elif method == "minmax_invert":
                data[:, dim_index] = VectorSpace.minmax_invert(
                    data[:, dim_index].reshape(-1, 1)
                ).flatten()
            elif method == "zscore_invert":
                data[:, dim_index] = VectorSpace.zscore_invert(
                    data[:, dim_index].reshape(-1, 1)
                ).flatten()
            elif method == "robust_invert":
                data[:, dim_index] = VectorSpace.robust_invert(
                    data[:, dim_index].reshape(-1, 1)
                ).flatten()
            else:
    ...
    def normalize_vectorspace(self, scaling_methods):
        """
        Normalize the coordinates of points based on the specified scaling methods for each dimension.
        Parameters:
        scaling_methods (dict): A dictionary specifying the scaling method for each dimension.
        ...
        """
        data = np.array([point.coordinates for point in self.points.values()])
        data = VectorSpace.normalize_dimensions(data, scaling_methods)
        for i, point in enumerate(self.points.values()):
            point.coordinates = data[i].tolist()
    ...
    def from_csv(
        ...
        with open(filepath, "r") as file:
            ...
            # Apply different scaling methods for each dimension if specified
            if scaling_methods:
                coords = VectorSpace.normalize_dimensions(coords, scaling_methods)
            else:
                # Default to minmax scaling if no methods are specified
                coords = VectorSpace.minmax(coords)
            vector_space = VectorSpace(dimensions)
